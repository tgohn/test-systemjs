...
function makeNonRelative(src) {
  return src.replace(/require\('.\//g, 'require(\'source-map/').replace(/\.\.\/\.\.\/lib\//g, '');
}
makeNonRelative.onRead = true;
function buildBrowser() {
  console.log('\nCreating dist/source-map.js');
  var project = copy.createCommonJsProject({roots: [path.join(__dirname, 'lib')]});
  copy({
    ...
    filter: [copy.filter.moduleDefines, removeAmdefine],
...
function buildFirefox() {
  ...
  project = copy.createCommonJsProject({roots: [__dirname, path.join(__dirname, 'lib')]});
  copy({
  ...
  testFiles.forEach(function(testFile) {
    ...
    copy({
      source: ['build/test-prefix.js', path.join('test', 'source-map', testFile), 'build/test-suffix.js'],
      filter: [removeAmdefine, makeNonRelative, function(input, source) {
        return input.replace('define(', 'define("' + path.join('test', 'source-map', testFile.replace(/\.js$/, '')) + '", ["require", "exports", "module"], ');
