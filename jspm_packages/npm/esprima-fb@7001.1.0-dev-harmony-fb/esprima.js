...
(function(process) {
  (function(root, factory) {
    ...
    SyntaxTreeDelegate = {
      ...
      createFunctionDeclaration: function(id, params, defaults, body, rest, generator, expression, isAsync, returnType, parametricType) {
        var funDecl = {
      ...
      createFunctionExpression: function(id, params, defaults, body, rest, generator, expression, isAsync, returnType, parametricType) {
        var funExpr = {
    ...
    function parsePropertyFunction(options) {
      ...
      return markerApply(marker, delegate.createFunctionExpression(null, params, defaults, body, options.rest || null, options.generator, body.type !== Syntax.BlockStatement, options.async, options.returnType, options.parametricType));
    }
    ...
    function parseObjectTypeAnnotation(nullable) {
      ...
      while (!match('}')) {
        ...
        properties.push(markerApply(marker, delegate.createProperty('init', propertyKey, propertyTypeAnnotation, isMethod, false)));
        if (!match('}')) {
          if (match(',') || match(';')) {
            lex();
          } else {
    ...
    function parseParametricTypeAnnotation() {
      ...
      expect('>');
      return markerApply(marker, delegate.createParametricTypeAnnotation(paramTypes));
    }
    function parseTypeAnnotation(dontExpectColon) {
      ...
      if (lookahead.type === Token.Identifier) {
        ...
        while (lookahead.type === Token.Identifier || match('?')) {
          params.push(parseTypeAnnotatableIdentifier(true, true));
          if (!match(')')) {
            expect(',');
          }
        }
      ...
      return markerApply(marker, delegate.createTypeAnnotation(typeIdentifier, parametricType, params, returnType, nullable));
    }
    ...
    function parseTypeAnnotatableIdentifier(requireTypeAnnotation, canBeOptionalParam) {
      ...
      if (requireTypeAnnotation || match(':')) {
        ident = markerApply(marker, delegate.createTypeAnnotatedIdentifier(ident, parseTypeAnnotation()));
      }
      if (isOptionalParam) {
    ...
    function parseParam(options) {
      ...
      if (match('[')) {
        ...
        param = rest ? parseVariableIdentifier() : parseTypeAnnotatableIdentifier(false, true);
    ...
    function parseFunctionDeclaration() {
      ...
      state.yieldAllowed = previousYieldAllowed;
      state.awaitAllowed = previousAwaitAllowed;
      return markerApply(marker, delegate.createFunctionDeclaration(id, tmp.params, tmp.defaults, body, tmp.rest, generator, false, isAsync, tmp.returnType, parametricType));
    }
    function parseFunctionExpression() {
      ...
      state.yieldAllowed = previousYieldAllowed;
      state.awaitAllowed = previousAwaitAllowed;
      return markerApply(marker, delegate.createFunctionExpression(id, tmp.params, tmp.defaults, body, tmp.rest, generator, false, isAsync, tmp.returnType, parametricType));
    }
    ...
    function parseMethodDefinition(existingPropNames) {
      ...
      return markerApply(marker, delegate.createMethodDefinition(propType, '', key, parsePropertyMethodFunction({
        generator: false,
        async: isAsync,
        parametricType: parametricType
      })));
    ...
    function parseClassProperty(existingPropNames) {
      ...
      expect(';');
      return markerApply(marker, delegate.createClassProperty(propertyIdentifier));
    }
