...
(function(process) {
  ...
  var createHash = require("github:jspm/nodelibs@0.0.7/crypto").createHash;
  ...
  ModuleReader.prototype = {
    ...
    readModuleP: util.cachedMethod(function(id) {
      ...
      return reader.getSourceP(id).then(function(source) {
        ...
        var hash = createHash("sha1").update("module\0").update(id + "\0").update(reader.salt + "\0").update(source.length + "\0" + source).digest("hex");
        if (reader.idToHash.hasOwnProperty(id)) {
          assert.strictEqual(reader.idToHash[id], hash, "more than one module named " + JSON.stringify(id));
        } else {
    ...
    buildModuleP: util.cachedMethod(function(id, hash, source) {
      var reader = this;
      return reader.processOutputP(id, hash, source).then(function(output) {
      ...
      function buildP() {
        ...
        reader.processors.forEach(function(build) {
          promise = promise.then(function(input) {
            return util.waitForValuesP(build.call(reader.context, id, input));
          });
        });
        return promise.then(function(output) {
          ...
          util.log.err("built Module(" + JSON.stringify(id) + ")", "cyan");
          return output;
        }).catch(function(err) {
  ...
  function warnMissingModule(id) {
    util.log.err("unable to resolve module " + JSON.stringify(id) + "; false positive?", "yellow");
    ...
    writeVersionP: function(outputDir) {
      ...
      return Q.all(Object.keys(output).map(function(key) {
        ...
        if (cacheDir) {
          ...
          return util.linkP(cacheFile, outputFile).catch(writeCopy);
        }
