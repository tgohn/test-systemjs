...
(function(Buffer, process) {
  ...
  exports.cachedMethod = function(fn, keyFn) {
    ...
    function wrapper() {
      ...
      return cache.hasOwnProperty(key) ? cache[key] : cache[key] = fn.apply(this, args);
    }
  ...
  function readFromStdinP(timeLimit, message, color) {
    ...
    var timeout = setTimeout(function() {
      log.err(message || ("Warning: still waiting for STDIN after " + timeLimit + "ms"), color || "yellow");
    }, timeLimit);
    try {
  ...
  exports.copyP = function(srcFile, dstFile) {
    return makePromise(function(callback) {
      ...
      function onError(err) {
        callback(err || new Error("error in util.copyP(" + JSON.stringify(srcFile) + ", " + JSON.stringify(dstFile) + ")"));
      }
      reader.on("error", onError).pipe(fs.createWriteStream(dstFile)).on("finish", function() {
  ...
  exports.inherits = function(ctor, base) {
    return ctor.prototype = Object.create(base.prototype, {constructor: {value: ctor}});
  };
...
})(require("github:jspm/nodelibs@0.0.7/buffer").Buffer, require("github:jspm/nodelibs@0.0.7/process"));
