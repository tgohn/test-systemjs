...
(function(process) {
  ...
  Cp.buildP = function(options, roots) {
    ...
    var waiting = 0;
    var output = outputDir ? new DirOutput(outputDir) : new StdOutput;
    if (self.watch) {
    ...
    function rebuild() {
      ...
      if (self.preferredFileExtension)
        context.setPreferredFileExtension(self.preferredFileExtension);
      ...
      return new ModuleReader(context, self.resolvers, self.processors).readMultiP(context.expandIdsOrGlobsP(roots)).then(context.ignoreDependencies ? pass : collectDepsP).then(outputModules).then(outputDir ? printModuleIds : pass).then(finish, function(err) {
        log.err(err.stack);
        if (!self.watch) {
          throw err;
        }
    ...
    return (outputDir ? util.mkdirP : Q)(outputDir).then(rebuild);
  };
  ...
  function collectDepsP(rootModules) {
    ...
    return Q.all(rootModules.map(traverse)).then(function() {
      return modules;
    });
  }
  ...
  function cliBuildP(commoner, version) {
    ...
    if (argc === 0) {
      if (options.config === true) {
        log.err("Cannot read --config from STDIN when reading " + "source from STDIN");
        process.exit(-1);
      }
      ...
      if (argc === 1) {
        ...
        outputDir = null;
        roots = [firstId];
        commoner.forceResolve(firstId, util.readFileP(first, commoner.sourceCharset));
        commoner.ignoreDependencies = true;
    ...
    if (options.cacheDir === false) {} else if (typeof options.cacheDir === "string") {
      commoner.cacheDir = absolutePath(workingDir, options.cacheDir);
    } else if (outputDir) {
      commoner.cacheDir = path.join(outputDir, ".module-cache");
    }
    var promise = getConfigP(workingDir, options.config).then(function(config) {
      var cleanOptions = {};
  ...
  function normalizeCharset(charset) {
    charset = charset && charset.replace(/[- ]/g, "").toLowerCase() || "utf8";
    assert.ok(iconv.encodingExists(charset), "Unrecognized charset: " + charset);
    return charset;
  }
  ...
  function getConfigP(workingDir, configFile) {
    if (typeof configFile === "undefined")
      return Q({});
    if (configFile === true || configFile === "<stdin>" || configFile === "-" || configFile === path.sep + path.join("dev", "stdin")) {
      return util.readJsonFromStdinP(1000, "Expecting configuration from STDIN (pass --config <file> " + "if stuck here)...", "yellow");
    }
