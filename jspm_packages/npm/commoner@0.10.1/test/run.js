...
(function(process) {
  ...
  function waitForHelpers(done, helperP, contextOptions) {
    Q.all([helperP(getNewDebugContext(contextOptions)), helperP(getNewReleaseContext(contextOptions))]).done(function() {
      done();
    });
  }
  ...
  describe("ModuleReader", function() {
    ...
    it("should cache previously-read modules", function(done) {
      var reader = new ModuleReader(debugContext, [getSourceP], []);
      var homes = [reader.readModuleP("home"), reader.readModuleP("home"), reader.readModuleP.originalFn.call(reader, "home")];
    ...
    it("should allow multiple file output", function(done) {
      ...
      function processP1(id, source) {
        ...
        return {
          ".js": source,
          ".json": util.readJsonFileP(path.join(__dirname, "..", "package.json"))
        };
      }
      ...
      var reader = new ModuleReader(debugContext, [getSourceP], [processP1, processP2, processP3]);
  ...
  describe("grepP", function() {
    it("should find @providesModule identifiers", function(done) {
      Q.spread([grepP("@providesModule\\s+\\S+", sourceDir), debugContext.getProvidedP()], function(pathToMatch, valueToPath) {
        assert.deepEqual(pathToMatch, {
          "widget/share.js": "@providesModule WidgetShare",
          "widget/.bogus.js": "@providesModule WidgetShare",
          "widget/bogus.js~": "@providesModule WidgetShare"
        });
        assert.deepEqual(valueToPath, {"WidgetShare": "widget/share.js"});
      }).done(done);
    });
  });
  describe("@providesModule", function() {
    it("should parse docblocks correctly", function(done) {
      ...
      assert.strictEqual(code.split("@provides" + "Module").length, 4);
      assert.strictEqual(debugContext.getProvidedId(code), "foo/bar");
      assert.strictEqual(debugContext.getProvidedId("no at-providesModule, here"), null);
      function helper(context) {
        var reader = new ModuleReader(context, [getProvidedP, getSourceP], []);
        return Q.all([Q.spread([reader.readModuleP("widget/share"), reader.readModuleP("WidgetShare")], function(ws1, ws2) {
          assert.strictEqual(ws1.id, ws2.id);
          assert.strictEqual(ws1.id, "WidgetShare");
          assert.strictEqual(ws1, ws2);
        }), reader.readMultiP(["widget/share", "WidgetShare"]).then(function(modules) {
          assert.strictEqual(modules.length, 1);
          assert.strictEqual(modules[0].id, "WidgetShare");
        }), reader.readModuleP("widget/gallery").then(function(gallery) {
          return gallery.getRequiredP();
        }).then(function(deps) {
          assert.strictEqual(deps.length, 1);
          assert.strictEqual(deps[0].id, "WidgetShare");
        }), Q.spread([reader.getSourceP("widget/share"), reader.getSourceP("WidgetShare")], function(source1, source2) {
  ...
  describe("util.relativize", function() {
    it("should resolve correct relative module identifiers", function(done) {
      ...
      function helperP(requiredId, expected) {
        assert.strictEqual(util.relativize(moduleId, requiredId), expected);
        return processor(moduleId, makeSource(requiredId)).then(function(output) {
          assert.deepEqual(output, {".js": makeSource(expected)});
        });
      }
      Q.all([helperP("another/nested/module", "../../../another/nested/module"), helperP("../../buried/module/./file", "../../buried/module/file"), helperP("../../buried/module/../file", "../../buried/file"), helperP("./same/level", "./same/level"), helperP("./same/./level", "./same/level"), helperP("./same/../level", "./level"), helperP("some/deeply/buried/treasure", "../buried/treasure"), helperP("./file", "./file"), helperP("./file/../../../module", "../../module"), helperP("./file/../../module", "../module")]).done(function() {
        done();
      });
    });
  });
  describe("canonical module identifiers", function() {
    it("should be returned by reader.getCanonicalIdP", function(done) {
      function helperP(context) {
        var reader = new ModuleReader(context, [getProvidedP, getSourceP], []);
        return Q.spread([reader.getCanonicalIdP("widget/share"), reader.getCanonicalIdP("WidgetShare"), reader.readModuleP("widget/share").get("id"), reader.readModuleP("WidgetShare").get("id")], function(ws1, ws2, ws3, ws4) {
    ...
    it("should replace non-canonical required identifiers", function(done) {
      function helperP(context) {
        ...
        return reader.readModuleP("widget/follow").then(function(follow) {
          ...
          assert.strictEqual(strCount('require("../WidgetShare")', follow.source), 4);
          assert.strictEqual(strCount('require("./gallery")', follow.source), 2);
          assert.strictEqual(strCount('require("../assert")', follow.source), 2);
        });
      }
      waitForHelpers(done, helperP);
    });
  ...
  describe("util.testWriteFd", function() {
    it("should write non-ASCII file data", function(done) {
      ...
      Q("ignored").then(check("x")).then(check("x\ny")).then(check("x\r\ny")).then(check("\t\nx\t")).then(check("\ufeff")).then(check("\u2603")).then(check("\ud83d\udc19")).finally(function() {
  ...
  describe("Watcher", function() {
    it("should basically work", function(done) {
      ...
      util.unlinkP(dummyFile).then(function() {
        ...
        return Q.all([watcher.readFileP("home.js"), watcher.readFileP(dummy)]);
      }).then(function() {
        return Q.all([util.unlinkP(dummyFile), waitForChangeP()]);
      }).done(function() {
        watcher.close();
        done();
      });
